...
class CallService {
  ...
  sendUpdateRecordingStateSystemMessage = (participantIds, recordingState) => {
    const msg = {
      extension: {
        janusRoomId: this.janusRoomId,
        callRecording: recordingState ? '1' : '0'
      }
    }
    return participantIds.map(user_id => ConnectyCube.chat.sendSystemMessage(user_id, msg))
  }
  ...
  onSystemMessage = msg => {
    ...
    } else if (extension.callRecording) {
      const { janusRoomId, callRecording } = extension
      if (this.janusRoomId === janusRoomId) {
        const normalizedRecordingState = callRecording === '1'
        this.onUpdateRecordingStateListener(normalizedRecordingState)
      }
    }
  }
  ...
  onUpdateRecordingStateListener(recordingState) {
    this.meeting['record'] = recordingState
    this.updateUIOnChangeMeetingRecordingState()
  }
  ...
  initGuestRoom = (janusRoomId = null) => {
    const currentUserSession = JSON.parse(localStorage.getItem(AuthService.constructor.CURRENT_USER_SESSION))
  ...
  createMeeting() {
    const params = { name: `${Math.random()}`, public: true }
  ...
  toggleMeetingRecod() {
    ...
    return ConnectyCube.meeting.update(this.meeting['_id'], params)
      .then(meeting => {
        this.meeting = meeting
        this.updateUIOnChangeMeetingRecordingState()
        const participantsIds = this.conectedParticipantIds.filter(userId => userId !== this.currentUserID)
        this.sendUpdateRecordingStateSystemMessage(participantsIds, this.meeting['record'])
      })
  }

  updateUIOnChangeMeetingRecordingState() {
    const recordButtonElm = document.getElementById('videochat-record-call')
    if (this.meeting['record']) {
      recordButtonElm.classList.add('on')
    } else {
      recordButtonElm.classList.remove('on')
    }
    const recordingOverlayElm = document.getElementById('recording-overlay')
    if (this.meeting['record']) {
      recordingOverlayElm.classList.remove('hidden')
    } else {
      recordingOverlayElm.classList.add('hidden')
    }
  }
}
