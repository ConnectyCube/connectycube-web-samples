...
export class DialogComponent implements OnInit, OnChanges {
  ...
  public selectedConversation$ = this.store$.select(
          ...
          const participants: Array<participant> = result.items.map(
            (user: any) => {
              return {
                ...
                login: user.login,
                avatar: user.avatar,
                me: user.login === atob(<string>localStorage.getItem('login')),
              };
            }
          );
          this.itemLastMessage =
            participants[0].full_name + ': ' + this.lastMessage;
  ...
  public getLastMessageUserName(participantId: number | null) {
    ...
    if (this.type === 2) {
      this.store$
        .select(getLastMessageParticipantName, { participantId })
        .pipe(
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (
      changes.lastMessage &&
      changes.lastMessage.currentValue !== changes.lastMessage.previousValue
    ) {
      const participantId = changes.lastMessageUserId
        ? changes.lastMessageUserId.currentValue
