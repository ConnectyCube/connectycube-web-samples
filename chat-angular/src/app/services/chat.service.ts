...
export class ChatService {
  ...
  private getChatParticipants(participantsIds: Array<number>) {
    ...
    participantsIds.forEach((id: number) => {
      this.store$
        .select(getParticipant, { participantId: id })
        .pipe(take(1))
        .subscribe((p) => {
          if (!p) {
    ...
    const params = {
      id: {
        in: unfinedParticipants,
      },
    };
    if (unfinedParticipants.length > 0) {
      return ConnectyCube.users.getV2(params as any);
    }
  ...
  private processParticipants(users: any) {
    ...
    const participants: Array<participant> = users.map((user: any) => {
      return {
        id: user.id,
        full_name: user.full_name,
        login: user.login,
        avatar: user.avatar,
        me: loginMe === user.login,
      };
    });
  ...
  private processTypingStatus(
    ...
    if (isTyping) {
      this.store$
        .select(getParticipant, { participantId: userId })
        .pipe(take(1))
        .subscribe((participant) => {
          if (participant) {
            this.store$.dispatch(
              addTypingParticipant({ dialogId, participant })
            );
          }
        });
    } else {
  ...
  private prepareMessageWithAttachmentAndSend(
    file: any,
    dialog: Dialog,
    senderId: number,
    tempMsgId: string,
    ...
    if (dialog.type === 3) {
      this.store$
        ...
        .subscribe((userMe) => {
          if (userMe) {
            const opponentId = dialog.participantIds.find(
              (id: number) => id !== userMe.id
            );
    ...
    if (message.id) {
      ...
      if (message.photo) {
        this.store$.dispatch(
          updateMessageId({
            dialogId: dialog.id,
            currentMsgIds: tempMsgId,
            newMsgIds: message.id,
          })
        );
        this.store$.dispatch(
          updateMessagePhoto({
  ...
  public init() {
      ...
      this.store$
        ...
        .subscribe((userMe) => {
          if (userMe) {
            if (userMe.id !== userId) {
              this.store$
                .select(selectedConversationSelector)
                .pipe(take(1))
      ...
      this.store$
        ...
        .subscribe((p) => {
          if (p !== undefined && !p.me) {
            ...
            if (
              msg.extension.hasOwnProperty('attachments') &&
              msg.extension.attachments.length > 0
            ) {
    ...
    ConnectyCube.chat.onSystemMessageListener = (msg: any) => {
      ...
      switch (msg.body) {
        ...
        case 'dialog/REMOVE_DIALOG_PARTICIPANT':
          this.store$.dispatch(
            updateDialogParticipant({ dialogId, userId: msg.userId })
          );
          break;
        case 'dialog/ADD_DIALOG_PARTICIPANTS':
          const addedParticipantsIds = msg.extension.addedParticipantsIds
            .split(',')
            .map(Number);
          this.searchUsersById(addedParticipantsIds)
            .then((result: any) => {
              if (result.items.length !== 0) {
                const participants: Array<participant> = result.items.map(
                  (user: any) => {
                    return {
                      id: user.id,
                      full_name: user.full_name,
                      ...
                      me:
                        user.login ===
                        atob(<string>localStorage.getItem('login')),
                    };
                  }
                );
                this.store$.dispatch(addParticipants({ participants }));
          ...
          this.store$.dispatch(
            addDialogParticipants({ dialogId, userIds: addedParticipantsIds })
          );
          break;
        case 'dialog/UPDATE_DIALOG_PARTICIPANTS':
          const updatedParticipants = msg.extension.updatedParticipants
            .split(',')
            .map(Number);
          this.store$.dispatch(
            updateDialogParticipants({ dialogId, userIds: updatedParticipants })
          );
          break;
    ...
    ConnectyCube.chat.onMessageTypingListener = (
      ...
      //Individual dialog
        ...
        this.store$
          .select(getIndividualDialogByParticipantId, { pId: userId })
          .pipe(take(1))
    ...
    ConnectyCube.chat.onSentMessageCallback = (
      ...
      this.store$.dispatch(
        updateMessageStatus({
          msgId: messageSent.id,
          status: messageSent ? 'sent' : 'error',
        })
      );
    };
      ...
      this.store$
        ...
        .subscribe((selectedConversation) => {
          if (selectedConversation === dialogId) {
            if (messageId) {
              this.store$.dispatch(
                updateMessageStatus({ msgId: messageId, status: 'read' })
              );
            } else {
              this.store$
                .select(getUnreadMessageListByStatus(dialogId))
                .pipe(take(1))
                .subscribe((msgIds) => {
                  if (msgIds.length !== 0) {
                    this.store$.dispatch(
                      updateMessagesStatus({ msgIds, status: 'read' })
                    );
                  }
                });
            }
  ...
  public getDialogs() {
    ConnectyCube.chat.dialog
      ...
      .then((result: any) => {
        ...
        this.searchUsersById(lastMessageUserIds)
          .then((result: any) => {
            if (result.items.length > 0) {
              const participants: Array<participant> = result.items.map(
                (user: any) => {
  ...
  public sendMsgWithPhoto(file: any, dialog: Dialog, senderId: number) {
    ...
    this.measureService.measureImage(urlImage).then(({ height, width }) => {
      ...
      this.store$.dispatch(
        addMessageId({ dialogId: dialog.id, msgIds: [message.id] })
      );
      this.store$.dispatch(addMessage({ message }));
      ...
      ConnectyCube.storage
        ...
        .then((result: any) => {
          console.log(result);
          this.prepareMessageWithAttachmentAndSend(
            result,
            dialog,
            senderId,
            message.id,
  ...
  public searchUsersByFullName(fullName: string) {
    const searchParams = { full_name: { start_with: fullName } };
    return ConnectyCube.users.getV2(searchParams);
  }
  ...
  public searchUsersByLogin(login: string) {
    const searchParams = { login: { start_with: login } };
    return ConnectyCube.users.getV2(searchParams as any);
  }

  public searchMethod(
    searchForm: any,
    selectedParticipant: Array<participant>
  ) {
    if (searchForm.valid) {
      ...
      Promise.allSettled([promiseF, promiseL]).then((result: any) => {
        ...
        result.forEach((item: any) => {
          if (item.hasOwnProperty('value')) {
            item.value.items.forEach((user: any) => {
              participants.set(String(user.id), user);
            });
          }
        });
        const participantArray: Array<participant> = [];
        [...participants].forEach(([key, value]) => {
          const active = selectedParticipant.some(
            (p: participant) => p.id === value.id
          );
  ...
  public getChatHistory(dialog: Dialog, isActivated: boolean) {
    return new Promise<void>((resolve, reject) => {
      ...
      if (!isActivated) {
        this.getChatParticipants(dialog.participantIds)
          .then((result: any) => {
            if (result) {
            ...
            ConnectyCube.chat.message
              ...
              .then((result: any) => {
                ...
                const unfindedUserIds: Array<{
                  msgId: string;
                  userId: number;
                }> = [];
                msgs.forEach((m: any) => {
                  this.store$
                    .select(getParticipant, { participantId: m.sender_id })
                    .pipe(take(1))
                    .subscribe((p) => {
                      if (!p) {
                        unfindedUserIds.push({
                          userId: m.sender_id,
                          msgId: m._id,
                        });
                      }
                      ...
                      if (
                        m.hasOwnProperty('attachments') &&
                        m.attachments.length > 0
                      ) {
                        const fileUID = m.attachments[0].uid;
                        message.photo =
                          ConnectyCube.storage.privateUrl(fileUID);
                    ...
                    if (result.items.length > 0) {
                      ...
                      const updates = unfindedUserIds.map((item) => {
                        const participant: any = participants.find(
                          (p: participant) => p.id === item.userId
                        );
                        return {
                          id: item.msgId,
                      ...
                      this.store$.dispatch(
                        updateMessageSendersName({ updates })
                      );
                    }
                  })
                  .catch((error: any) => {
      ...
      this.store$
        ...
        .subscribe((userMe) => {
          if (userMe) {
            ...
            ConnectyCube.chat.sendIsTypingStatus(opponentId as number);
          }
        });
    }
  }
      ...
      this.store$
        ...
        .subscribe((userMe) => {
          if (userMe) {
            ...
            ConnectyCube.chat.sendIsStopTypingStatus(opponentId as number);
          }
        });
    }
  }
  ...
  public readAllChatMessages(dialogId: string) {
    ...
    this.store$
      .select(getUnreadMessageListByBadge(dialogId))
      .pipe(
        filter((v) => !!v),
        take(1)
      )
      .subscribe((unreadMessageList) => {
        if (unreadMessageList.length !== 0) {
          const userIds: Array<any> = [
            ...new Set(unreadMessageList.map((msg: Message) => msg.senderId)),
          ];
    ...
    ConnectyCube.chat.message.update(messageIds, params).catch(() => {});
  }
  ...
  public getLastActivity(userId: number) {
    if (ConnectyCube.chat.isConnected) {
      ...
      return ConnectyCube.chat.getLastUserActivity(userId);
    }
  ...
  public removeParticipantFromChat(
    dialogId: string,
    ocupantsIds: Array<number>
  ) {
    const toUpdateParams = { pull_all: { occupants_ids: ocupantsIds } };

    return ConnectyCube.chat.dialog.update(dialogId, toUpdateParams);
  }
