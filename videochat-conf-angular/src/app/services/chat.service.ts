...
import {
  findParticipantSelector,
  participantSelector,
} from '../reducers/participant.selectors';
...
import { Meetings, Users } from 'connectycube/dist/types/types';
...
export class ChatService {
  ...
  private processMessages(messagesInput: any) {
    return new Promise<any>((resolve, reject) => {
      const missedUsers: any = [];

      const messages = messagesInput.map((item: any) => {
        const message: any = { senderName: '', body: '', time: '' };
        // console.log("message item", item)
        message.body = this.sanitizer.sanitize(
          SecurityContext.HTML,
          CommonUtilities.escapeHTMLString(item.message)
        );
        message.time =
          new Date(item.date_sent * 1000).toLocaleDateString() +
          ' ' +
          new Date(item.date_sent * 1000).toLocaleTimeString().slice(0, 5);

        this.store$
          .select(findParticipantSelector, { userId: item.sender_id })
          .pipe(take(1))
          .subscribe((user) => {
            ...
            if (user) {
              ...
              if (missedUsers.indexOf(item.sender_id) === -1) {
                missedUsers.push(item.sender_id);
              }
      ...
      if (missedUsers.length > 0) {
        ...
        ConnectyCube.users
          .getV2(params)
          .then((response: Users.GetV2Response) => {
            response.items.forEach((user: Users.User) => {
              ...
              for (let i = 0; i < messages.length; i++) {
                ...
                if (
                  messages[i].IsUndefinedUserDetails &&
                  messages[i].userId === id
                ) {
  ...
  public init() {
    ConnectyCube.chat.onMessageListener = (userId: any, message: any) => {
      this.store$
        .select(findParticipantSelector, { userId })
        .pipe(take(1))
        .subscribe((user) => {
          if (!user.me) {
            ...
            const time =
              new Date(
                message.extension.date_sent * 1000
              ).toLocaleDateString() +
              ' ' +
              new Date(message.extension.date_sent * 1000)
                .toLocaleTimeString()
                .slice(0, 5);
            ...
            this.store$.dispatch(
              addMessage({
                ...
                body: this.sanitizer.sanitize(
                  SecurityContext.HTML,
                  CommonUtilities.escapeHTMLString(body)
                ),
                time: time,
              })
            );
    ...
    ConnectyCube.chat.onSystemMessageListener = (msg: any) => {
      ...
      switch (command) {
          ...
          this.store$
            ...
            .subscribe((res) => {
              if (res.some((user: User) => user.id === msg.userId)) {
                this.store$.dispatch(
                  updateVideoStatus({ id: msg.userId, videoStatus: true })
                );
              } else {
                this.store$.dispatch(
                  addUser({
        ...
        case 'VIDEO_OFF':
          ...
          this.store$
            ...
            .subscribe((res) => {
              if (res.some((user: User) => user.id === msg.userId)) {
                this.store$.dispatch(
                  updateVideoStatus({ id: msg.userId, videoStatus: false })
                );
              } else {
                this.store$.dispatch(
                  addUser({
        ...
        case 'SHARE_ON':
          this.store$.dispatch(
            updateVideoStatus({ id: msg.userId, videoStatus: 'share' })
          );
          break;
        case 'SHARE_OFF':
          this.store$.dispatch(
