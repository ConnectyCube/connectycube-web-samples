...
export class PrejoinComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.matIconRegistry.addSvgIcon(
      ...
      this.domSanitizer.bypassSecurityTrustResourceUrl(
        '../assets//icons/videocam_white_24dp.svg'
      )
    );
    this.matIconRegistry.addSvgIcon(
      'videocam_off',
      this.domSanitizer.bypassSecurityTrustResourceUrl(
        '../assets//icons/videocam_off_white_24dp.svg'
      )
    );
  }
  ...
  private checkMediaDevices() {
    return navigator.mediaDevices.enumerateDevices().then((dev: any) => {
      console.log('Join Devices', dev);
      const InputDevice = dev.filter((device: any) =>
        device.kind.includes('input')
      );
      console.log(InputDevice);
      this.isMicrophoneAvailable = InputDevice.some(
        (device: any) => device.kind === 'audioinput'
      );
      this.isCameraAvailable = InputDevice.some(
  ...
  private onVideo() {
    ...
    navigator.mediaDevices
      ...
      .then((localStream) => {
        ...
        this.callService.SetOurDeviceId(
      ...
      .catch((error: any) => {
        ...
        if (
          ...
          if (
            ...
            this.ModalOn(
              'Camera and Microphone Denied',
              this.messageErrorDeniedAll
            );
  ...
  public muteOrUnmuteVideo() {
    ...
    this.videoIconName =
      this.videoIconName === 'videocam' ? 'videocam_off' : 'videocam';
  }
          ...
          if (meetId && !this.previousUrl) {
            ...
            if (userName.trim()) {
              ...
              this.authService
                ...
                .then((user: any) => {
                  this.chatService
                    .subscribeToDialog(meetId)
                    .then((dialog: any) => {
                      console.warn(dialog._id);
                      this.store$.dispatch(
                        setActiveDialogId({ dialogId: dialog._id })
                      );
                      this.chatService
                        .getDialogHistory(dialog._id)
                        .then((dialogMessages: Array<Message>) => {
                  ...
                  this.callService
                    .joinUser(meetId, user.id, userName)
                    .then((obj: any) => {
                      this.DoorOpen = true;
                    })
                    .catch((error: any) => {
            ...
            if (userName.trim()) {
              ...
              this.authService.auth(userName).then((user) => {
                this.callService
                  .createMeetingAndJoin(user)
                  .then((roomUrl: string) => {
  ...
  ngOnInit(): void {
    ...
    try {
      this.checkPermissions().then(() => {
        ...
        this.store$.dispatch(
          addVideoPermission({
            videoPermission: this.videoPermission === 'granted',
          })
        );
        this.store$.dispatch(
          addAudioPermission({
            audioPermission: this.audioPermission === 'granted',
          })
        );
        this.onVideo();
      });
